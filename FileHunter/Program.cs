    ...
    class Program
    {
        ...
        public static void Director(string dir, List<string> list)
        {
            try
            {
                ...
                {
                    if (CheckFile(f.Name,f.FullName))
                    {
                        list.Add("File;" + f.FullName);
                    }
                }
                foreach (DirectoryInfo dd in directs)
                {
                    if (CheckFile(dd.Name, dd.Name))
                    {
                        list.Add("Dir;" + dd.FullName);
                    }
        ...
        {
            ".exe",".dll",".png",".jpg",".bmp",".xml",".bin",".dat",".manifest","locale","winsxs","windows\\sys"
        };

        public static string[] whitelist = new string[]
        {
            ".ini","pass","conf","account","login","pwd","user","oa","root","密码","账号","账户","配置","服务器","数据库","备忘","常用","通讯录"
        };

        public static bool CheckFile(string name, string fullname)
        {
            foreach (var item in blacklist)
            {
                if (fullname.ToLower().Contains(item))
                {
                    return false;
                }
            }
